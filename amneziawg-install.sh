...
install_awg_packages() {
    ...
    BASE_URL="https://github.com/behar666/awg-openwrt/releases/download/"
    ...
    if apk list --installed | grep -q kmod-amneziawg; then
        ...
        apk --allow-untrusted add "$AWG_DIR/$KMOD_AMNEZIAWG_FILENAME"
    ...
    if apk list --installed | grep -q amneziawg-tools; then
        ...
        apk --allow-untrusted add "$AWG_DIR/$AMNEZIAWG_TOOLS_FILENAME"
    ...
    if apk list --installed | grep -q "luci-proto-amneziawg\|luci-app-amneziawg"; then
        ...
        apk --allow-untrusted add "$AWG_DIR/$LUCI_AMNEZIAWG_FILENAME"
    ...
    if [ "$AWG_VERSION" = "2.0" ]; then
        ...
        if [ "$INSTALL_RU_LANG" = "y" ] || [ "$INSTALL_RU_LANG" = "Y" ]; then
            if apk list --installed | grep -q luci-i18n-amneziawg-ru; then
                ...
                if [ $? -eq 0 ]; then
                    ...
                    apk --allow-untrusted add "$AWG_DIR/$LUCI_I18N_AMNEZIAWG_RU_FILENAME"
                    if [ $? -eq 0 ]; then
